...
<body>
	...
	<div class="section">
		...
		<% 
				...
				try {
				    ...
				    while(rs.next()) {
				    	...
				    	out.print("    <td width='73%'><a href='javascript:notice_read("+rs.getString("no")+");'>"+rs.getString("title")+"</a></td>");
		...
		%>
		           <a href='javascript:notice_list(<%=i %>);'>[<%=i%>]</a>
		<%
		        };
		%>
				</td>
		...
		<form id="form2" action="notice_list.jsp" method="post">		
        	...
        	<div>            
				<table width='100%'>
					<tr>
						<td align="center">
                			<label for="searchColumn">검색</label>
                    		<select name="searchColumn" id="searchColumn" title="검색구분 선택" >
                        		<option <%  if (searchColumn.equals("title")) { out.print("selected");  } %> value="title">제목</option>
                            	<option <%  if (searchColumn.equals("contents")) { out.print("selected");  } %> value="contents">내용</option>
                        	</select>
                    		<input type="text" name="keyword" id="keyword" class="w300" maxlength="50" title="검색어 입력" value="<%=keyword%>"/>
                    		<input type="button" value="조회" onclick="notice_list('1')" >
						</td>
					</tr>
				</table>
       		</div>
