...
try{
   ...
   String sql2 = "select COUNT(*) from lossreport where 1=1 " + where;
   ...
   String sql = String.format("select * from lossreport WHERE 1=1 %s order by idx desc limit %d,%d;",where,pageNum,term);
...
<script>
	function fncSubmit() {
		...
		form.action ="Lossreport_board.jsp";
		form.submit();
	}
</script>
<div id="notice">분실 신고</div>
<br>
...
<div>
   ...
   %>
    <div class="sub">
        ...
        <div style="height: 18%">&nbsp;• 분실 지역: <%=rs2.getString("place") %> <%=rs2.getString("place1")%> <%=rs2.getString("place2")%> </div>
        <div style="height: 18%">&nbsp;• 분실 날짜: <%=rs2.getString("day")%></div>
	</div>
 <%    
   }
...
<% 
	...
	//if(prev){
		%>
	<%-- 	 <a href="Lossreport_board.jsp?pageCnt=<%=pageCnt-1 %>">prev</a>  --%>
	...
	for(int i=beginPage; i<=endPage; i++){
		%>
		<a href="Lossreport_board.jsp?pageCnt=<%=i %>"><%=i %></a>
		<%
	}
...
%>
 <button id="button" onclick="location.href='Lossreport_write.jsp'" style="margin-top: 40px;">글쓰기</button> 
</div>
<br>
<br>
<br>
