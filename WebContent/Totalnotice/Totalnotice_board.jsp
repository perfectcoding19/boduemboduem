...
<script type="text/javascript">
	function categoryChange(e) {
		var good_개 = [ "선택", "기타", "골든 리트리버", "그레이 하운드", "그레이트 덴", "닥스훈트",
				"달마시안", "도베르만", "말라뮤트", "말티즈", "푸들", "스탠다들 푸들", "토이 푸들", "믹스견",
				"보더콜리", "불독", "비글", "비숑 프리제", "사모예드", "삽살개", "슈나우저", "스피츠",
				"시바", "시베리안 허스키", "시츄", "요크셔 테리어", "웰시코기", "치와와", "포메라니안" ];
		var good_고양이 = [ "선택", "노르웨이 숲", "데본 렉스", "버말", "러시안 블루", "먼치킨", "믹스묘",
				"벵갈", "봄베이", "발리네즈", "사바나 캣", "샴", "스핑크스", "아메리칸 쇼트헤어", "페르시안",
				"한국고양이" ];
		var good_기타 = [ "선택", "기타 축종" ];
		var good_서울특별시 = [ "전체", "강남구", "강동구", "강북구", "강서구", "관악구", "광진구",
				"구로구", "금천구", "노원구", "도봉구", "동대문구", "동작구", "마포구", "서대문구",
				"서초구", "성동구", "성북구", "송파구", "양천구", "영등포구", "용산구", "은평구", "종로구",
				"중구", "중랑구" ];
		var good_인천광역시 = [ "전체", "강화군", "계양구", "남동구", "동구", "미추홀구", "부평구",
				"서구", "연수구", "옹진군", "중구" ];
		var good_대구광역시 = [ "전체", "남구", "달서구", "달성군", "동구", "북구", "서구", "수성구",
				"중구" ];
		var good_대전광역시 = [ "전체", "대덕구", "동구", "서구", "유성구", "중구" ];
		var good_광주광역시 = [ "전체", "광산구", "남구", "동구", "북구", "서구" ];
		var good_부산광역시 = [ "전체", "강서구", "금정구", "기장군", "남구", "동구", "동래구",
				"부산진구", "북구", "사상구", "사하구", "서구", "수영구", "연제구", "영도구", "중구",
				"해운대구" ];
		var good_울산광역시 = [ "전체", "남구", "동구", "북구", "울주군", "중구" ];
		var good_세종특별시 = [ "전체" ];
		var good_경기도 = [ "전체", "가평군", "고양시", "과천시", "광명시", "광주시", "구리시", "군포시",
				"김포시", "남양주시", "동두천시", "부천시", "성남시", "수원시", "시흥시", "안산시",
				"안성시", "안양시", "양주시", "양평군", "여주시", "연천군", "오산시", "용인시",
				"용인시 기흥구", "의왕시", "의정부시", "이천시", "파주시", "평택시", "포천시", "하남시",
				"화성시" ];
		var good_강원도 = [ "전체", "강릉시", "고성군", "동해시", "삼척시", "속초시", "양구군", "양양군",
				"영월군", "원주시", "인제군", "정선군", "철원군", "춘천시", "태백군", "평창군", "홍천군",
				"횡성군" ];
		var good_충청북도 = [ "전체", "괴산군", "단양군", "보은군", "영동군", "옥천군", "음성군",
				"제천시", "증평군", "진천군", "청주시", "충주시" ];
		var good_충청남도 = [ "전체", "계룡시", "공주시", "금산군", "논산시", "당진시", "보령시",
				"부여군", "서산시", "서천군", "아산시", "연기군", "예산군", "천안시", "청양군", "태안군",
				"홍성군" ];
		var good_전라북도 = [ "전체", "고창군", "군산군", "김제시", "남원시", "무주군", "부안군",
				"순창군", "완주군", "익산시", "임실군", "장수군", "전주시", "정읍시", "진안군" ];
		var good_전라남도 = [ "전체", "강진군", "고흥군", "곡성군", "광양시", "구례군", "나주시",
				"담양군", "목포시", "무안군", "보성군", "순천시", "신안군", "여수시", "영광군", "영암군",
				"완도군", "장성군", "장흥군", "진도군", "함평군", "해남군", "화순군" ];
		var good_경상북도 = [ "전체", "경산시", "경주시", "고령군", "구미시", "군위군", "김천시",
				"문경시", "봉화군", "상주시", "성주군", "안동시", "영덕군", "영양군", "영주시", "영천시",
				"예천군", "울릉군", "울진군", "의성군", "청도군", "청송군", "칠곡군", "포항시" ];
		var good_경상남도 = [ "전체", "거제시", "거창군", "고성군", "김해시", "남해군", "밀양시",
				"사천시", "산청군", "양산시", "의령군", "진주시", "창녕군", "창원 마산합포회원구",
				"창원 의창성산구", "창원 진해구", "통영시", "하동군", "함안군", "함양군", "합천군" ];
	...
	function popup_open(no) {
		window.open("Totalnotice_popup.jsp?no=" + no, "new", "width=" + _width
...
<body>
	...
	function fncSubmit() {
		...
		form.action ="Totalnotice_board.jsp";
		form.submit();
	}
</script>
	<div class="section">
		...
		<form name="search" action="javascript:fncSubmit()" method="get">
			<table id="warning">
				<tr>
					<td>&nbsp; &emsp;「동물보호법」 제17조, 시행령7조 및 동법 시행규칙 제20조에 따라
						유기·유실동물을 보호하고 있는 경우에는 소유자 등이 보호조치<br> &nbsp; &emsp; 사실을 알 수
						있도록 7일 동안 공고하여야 합니다.<br> &nbsp; &emsp; 공고 중인 동물 소유자는 해당 시군구 및
						동물보호센터에 문의하시어 동물을 찾아가시기 바랍니다.<br> &nbsp; &emsp; 다만, 「동물보호법」
						제19조 및 동법 시행규칙 제21조에 따라 소유자에게 보호비용이 청구될 수 있습니다.<br> &nbsp;
						&emsp; 또한 「동물보호법」 제17조에 따른 공고가 있는 날부터 10일이 경과하여도 소유자 등을 알 수 없는
						경우에는「유실물법」 제12조 및<br> &nbsp; &emsp; 「민법」 제253조의 규정에도 불구하고 해당
						시·도지사 또는 시장·군수·구청장이 그 동물의 소유권을 취득하게 됩니다.
					</td>
				</tr>
			</table>
			<br>
		...
		<div class="main">
			...
			<%
				//if(prev){
			%>
			<%-- 	 <a href="Totalnotice_board.jsp?pageCnt=<%=pageCnt-1 %>">prev</a>  --%>
			<div style="display: inline-block; margin-left: 50%">
				<%
					//}
				for (int i = beginPage; i <= endPage; i++) {
				%>
				<a style="color:black;" href="Totalnotice_board.jsp?pageCnt=<%=i%>"><%=i%></a>
				<%
					}
				%>
			</div>
			...
			%>
			<button id="button" onclick="location.href='Totalnotice_write.jsp'"
