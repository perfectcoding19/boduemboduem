...
try{
   ...
   String sql = String.format("select * from Totalnotice WHERE 1=1 %s order by idx desc limit %d,%d;",where,pageNum,term);
...
<table id="warning">
<tr>
 <td> &nbsp; &emsp;「동물보호법」 제17조, 시행령7조 및 동법 시행규칙 제20조에 따라 유기·유실동물을 보호하고 있는 경우에는 소유자 등이<br>
   &nbsp; &emsp; 보호조치 사실을 알 수 있도록 7일 동안 공고하여야 합니다.<br>
   &nbsp; &emsp; 공고 중인 동물 소유자는 해당 시군구 및 동물보호센터에 문의하시어 동물을 찾아가시기 바랍니다.<br>
   &nbsp; &emsp; 다만, 「동물보호법」 제19조 및 동법 시행규칙 제21조에 따라 소유자에게 보호비용이 청구될 수 있습니다.<br>
   &nbsp; &emsp; 또한 「동물보호법」 제17조에 따른 공고가 있는 날부터 10일이 경과하여도 소유자 등을 알 수 없는 경우에는<br> 
   &nbsp; &emsp;「유실물법」 제12조 및 「민법」 제253조의 규정에도 불구하고 해당 시·도지사 또는 시장·군수·구청장이 그 동물의 소유권을 취득하게 됩니다. </td>
</tr>
</table>
<br>
	<table>
...
<div>
   ...
   %>
    <div class="sub">
        ...
        <div style="height: 18%">&nbsp;• 지역: <%=rs2.getString("place") %> <%=rs2.getString("place1")%> <%=rs2.getString("place2")%> </div>
        <div style="height: 18%">&nbsp;• 날짜: <%=rs2.getString("day")%></div>
	</div>
 <%    
   }
...
<% 
	...
	for(int i=beginPage; i<=endPage; i++){
		%>
		<a href="Totalnotice_board.jsp?pageCnt=<%=i %>"><%=i %></a>
		<%
	}
...
%>
 <button id="button" onclick="location.href='Totalnotice_write.jsp'" style="margin-top: 40px;">글쓰기</button> 
</div>
<br>
<br>
<br>
